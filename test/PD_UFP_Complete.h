/**
 * PD_UFP_Complete.h - FUSB302 Bridge模式完整修复版本头文件
 *
 * 修复内容：
...
struct pd_monitor_t {
    ...
    uint16_t last_voltage;          // 最后一次电压（mV）
    uint16_t last_current;          // 最后一次电流（mA）
...
class PD_UFP_c
{
    ...
    protected:
        ...
        void handle_FUSB302_bridge_event(FUSB302_event_t events); // 新增：Bridge专用事件处理
        bool timer(void);
    ...
    bool bridge_mode_confirmed;     // 新增：Bridge模式确认标志
    bool monitor_active;           // 新增：监听活跃状态
...
class PD_UFP_Log_c : public PD_UFP_c
{
    public:
        PD_UFP_Log_c(pd_log_level_t log_level = PD_LOG_LEVEL_INFO, 
                     pd_bridge_log_level_t bridge_log_level = PD_BRIDGE_LOG_LEVEL_DETAILED);
        // Task
